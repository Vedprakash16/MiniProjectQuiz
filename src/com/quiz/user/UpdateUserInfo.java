...
public class UpdateUserInfo {
		...
		public void updateUser(int userId, String username, String email, String password, String city, long mobile) {
			try {
				query = "update user set username=? ,email=?,password=?,city=?,mobile=? whereuserid=?";
	...
	public void updateUser(String username, int userId) {
			try {
				query = "update user set username=? whereuserid=? ";
				ps = con.prepareStatement(query);
				ps.setString(1, username);
				ps.setInt(2, userId);
				if (ps.execute() == false) {
	...
	public void updateUser(int userId, String email, String password) {
			try {
				query = "update user set email=?,password=? whereuserid=? ";
				ps = con.prepareStatement(query);
		...
		public void updateUser(int userId, String city) {
			try {
				query = "update user set city=? whereuserid=? ";
				ps = con.prepareStatement(query);
				ps.setString(1, city);
				ps.setInt(2, userId);
				if (ps.execute() == false) {
		...
		public void updateUser(int userId, long mobile) {
			try {
				query = "update user set mobile=? whereuserid=? ";
				ps = con.prepareStatement(query);
				ps.setLong(1, mobile);
				ps.setInt(2, userId);
				if (ps.execute() == false) {
